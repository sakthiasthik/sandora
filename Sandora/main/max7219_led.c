...
// Internal helper to send a command to all devices
static void max7219_send_command(uint8_t address, uint8_t value) 
{
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));

    uint8_t data[MAX7219_NUM_DEVICES * 2];
...
// Internal helper to send command to a specific device
static void max7219_send_to_device(uint8_t device, uint8_t address, uint8_t value) 
{
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    ...
    // Note: device index starts from 0 (leftmost device)
    data[(MAX7219_NUM_DEVICES - 1 - device) * 2] = address;
    data[(MAX7219_NUM_DEVICES - 1 - device) * 2 + 1] = value;
...
esp_err_t max7219_init(void) 
{
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = MAX7219_NUM_DEVICES * 2
    };
    ...
    max7219_send_command(MAX7219_DISPLAY_TEST, 0x00);  // Disable test mode
    max7219_send_command(MAX7219_SCAN_LIMIT, 0x07);    // Display all digits
    max7219_send_command(MAX7219_DECODE_MODE, 0x00);   // Use matrix mode (no 7-seg decoding)
    max7219_send_command(MAX7219_SHUTDOWN, 0x01);      // Exit shutdown mode
    max7219_set_intensity(8);                          // Default brightness
    max7219_clear();
...
void max7219_set_intensity(uint8_t intensity) 
{
    ...
    max7219_send_command(MAX7219_INTENSITY, intensity);
}
...
void max7219_draw_row(uint8_t device, uint8_t row, uint8_t value) 
{
    if (row < 1 || row > 8 || device >= MAX7219_NUM_DEVICES) return;
    max7219_send_to_device(device, row, value);
}

void max7219_draw_matrix(uint8_t device, const uint8_t *data) 
{
    if (device >= MAX7219_NUM_DEVICES) return;
    for (uint8_t row = 0; row < 8; ++row) {
        max7219_draw_row(device, row + 1, data[row]);
    }
}
